...
def pad_sents(sents, pad_token):
    ...
    max_len = max(len(sent) for sent in sents)  # Find the length of the longest sentence
    sents_padded = [[pad_token] * max_len for _ in range(len(sents))]  # Initialize padded sentences
    for i, sent in enumerate(sents):
        sents_padded[i][:len(sent)] = sent  # Copy the original sentence into the padded one
    ### END CODE HERE
