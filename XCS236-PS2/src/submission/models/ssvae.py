...
class SSVAE(nn.Module):
    ...
    def negative_elbo_bound(self, x):
        ...
        # KL divergence for y
        log_q_y = y_logprob
        log_p_y = torch.log(torch.tensor(1.0 / self.y_dim, device=x.device))
        kl_y = torch.sum(y_prob * (log_q_y - log_p_y), dim=1).mean()

        # Duplicate y based on x's batch size
        y = np.repeat(np.arange(self.y_dim), x.size(0))
        y = x.new(np.eye(self.y_dim)[y])
        x_repeated = ut.duplicate(x, self.y_dim)

        # Compute q(z|x,y)
        z_mean, z_var = self.enc(torch.cat([x_repeated, y], dim=-1))
        ...
        rec = -ut.log_bernoulli_with_logits(x_repeated, logits).view(self.y_dim, x.size(0), -1).sum(-1)

        # Compute KL divergence for z
        kl_z = ut.kl_normal(z_mean, z_var, self.z_prior_m.expand_as(z_mean), self.z_prior_v.expand_as(z_var)).view(self.y_dim, x.size(0))

        # Compute the expected values with respect to q(y|x)
        rec = (y_prob.t() * rec).sum(0).mean()
        kl_z = (y_prob.t() * kl_z).sum(0).mean()
