...
class MultiTaskNet(nn.Module):
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        # Factorization task
        predictions = (user_embeddings * item_embeddings).sum(dim=1) + user_bias + item_bias

        # Regression task
        interaction_vector = torch.cat([user_embeddings, item_embeddings, user_embeddings * item_embeddings], dim=1)
        score = interaction_vector
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        # Factorization task
        predictions = (user_embeddings_fact * item_embeddings_fact).sum(dim=1) + user_bias + item_bias

        # Regression task
        interaction_vector = torch.cat([user_embeddings_reg, item_embeddings_reg, user_embeddings_reg * item_embeddings_reg], dim=1)
        score = interaction_vector
        for layer in self.mlp_layers:
            score = layer(score)
        score = score.squeeze()
