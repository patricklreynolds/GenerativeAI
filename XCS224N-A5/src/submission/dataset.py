...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Determine the length of the masked content (random, on average 1/4 the length of the truncated document)
        avg_mask_length = max(1, int(truncated_length * self.masking_percent))
        mask_length = random.randint(1, 2 * avg_mask_length)

        # Ensure the mask length does not exceed the truncated length
        mask_length = min(mask_length, truncated_length - 1)
        
        # Choose a random start position for the mask
        mask_start = random.randint(0, truncated_length - mask_length)
        ...
        # Form the masked string as per the specification
        ...
        # Convert input and output sequences to Long tensors
        x = torch.tensor([self.stoi[c] for c in x], dtype=torch.long)
        y = torch.tensor([self.stoi[c] for c in y], dtype=torch.long)
