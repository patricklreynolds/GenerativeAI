...
class DataGenerator(IterableDataset):
    ...
    def _sample(self, sample_fn=None, shuffle_fn=None):
        ...
        #############################
        ### START CODE HERE ###
        # Sample N different characters
        # Sample N different characters
        selected_folders = sample_fn(self.folders, self.num_classes)
        
        # Load K + 1 images per character and collect associated labels
        labels_and_images = get_images(selected_folders, list(range(self.num_classes)), self.num_samples_per_class)
        
        images = np.zeros((self.num_samples_per_class, self.num_classes, self.dim_input), dtype=np.float32)
        labels = np.zeros((self.num_samples_per_class, self.num_classes, self.num_classes), dtype=np.float32)

        for idx, (label, image_path) in enumerate(labels_and_images):
            image = self.image_file_to_array(image_path, self.dim_input)
            i = idx % self.num_samples_per_class
            j = idx // self.num_samples_per_class
            images[i, j, :] = image
            labels[i, j, label] = 1
        
        # Shuffle the query set (last set of images and labels)
        shuffle_fn(images[-1])
        shuffle_fn(labels[-1])
