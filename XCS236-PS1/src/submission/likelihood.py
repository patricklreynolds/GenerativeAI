...
def log_likelihood(model, text):
    ...
    with torch.no_grad():
        ...
        ### START CODE HERE ###
        ### START CODE HERE ###
        outputs = model(text)  # Obtain outputs from the model
        if isinstance(outputs, tuple):  # Check if the output is a tuple and extract logits
            logits = outputs[0]
        else:
            logits = outputs.logits

        # Prepare the target labels by shifting the input text tensor to the right
        shifted_logits = logits[:, :-1, :]
        shifted_labels = text[:, 1:]

        # Set up the cross-entropy loss function
        criterion = nn.CrossEntropyLoss(reduction='sum')
        # Compute the loss as the sum of negative log likelihoods across the sequence
        loss = criterion(shifted_logits.reshape(-1, shifted_logits.size(-1)), shifted_labels.reshape(-1))

        # Convert loss to positive log likelihood
